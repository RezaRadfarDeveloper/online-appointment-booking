...
router.beforeEach((to, from, next) => {
  ...
  if (to.meta.requiresAuth && !isLoggedIn.value) {
    ...
    console.log('not auth', isLoggedIn.value)
    ...
    // If the user is already authenticated and tries to access the login page, redirect to confirmation
    ...
    next({ name: 'home' })
  } else if (from.name === 'confirmation' && isLoggedIn.value && to.name === 'confirmation') {
    // If the user has come to auth page but not selected appointment(step 1 is left)then go back to step 1
    next({ name: 'confirmation' })
  } else {
    // Allow navigation
